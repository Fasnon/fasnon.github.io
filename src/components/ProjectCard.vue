<script setup lang="ts">
import type { Project } from '../data/projects'
import StackTag from './StackTag.vue'

defineProps<{ project: Project; imageFirst: boolean }>()
</script>

<template>
  <article
    class="group overflow-hidden rounded-lg shadow-sm bg-white motion-safe:hover:-translate-y-2 hover:shadow-md transition-all duration-300"
    :class="['flex', imageFirst ? 'flex-col' : 'flex-col-reverse']"
  >
    <!-- Image / Placeholder area -->
    <div class="overflow-hidden aspect-video">
      <img
        v-if="project.image"
        :src="project.image"
        :alt="project.title"
        loading="lazy"
        class="w-full h-full object-cover motion-safe:group-hover:scale-105 transition-transform duration-500"
      />
      <div
        v-else
        class="w-full h-full flex items-center justify-center bg-gradient-to-br from-muted-green to-muted-peach"
      >
        <img
          v-if="project.placeholderIcon"
          :src="project.placeholderIcon"
          :alt="project.title + ' icon'"
          class="h-16 w-16 opacity-60"
        />
      </div>
    </div>

    <!-- Content area -->
    <div class="flex flex-col flex-1 p-5 gap-3">
      <h3 class="font-semibold text-xl text-mahogany">{{ project.title }}</h3>
      <p class="text-sm text-mahogany/80 leading-relaxed">{{ project.description }}</p>

      <!-- Tags -->
      <div class="flex flex-wrap gap-2 mt-auto pt-3">
        <StackTag
          v-for="tag in project.tags"
          :key="tag.label"
          :label="tag.label"
        />
      </div>

      <!-- Links -->
      <div class="flex gap-3 pt-2">
        <a
          v-if="project.links.live"
          :href="project.links.live"
          target="_blank"
          rel="noopener noreferrer"
          class="p-2 text-mahogany/60 hover:text-mahogany transition-colors duration-200"
          aria-label="Live demo"
        >
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
          </svg>
        </a>
        <a
          v-if="project.links.repo"
          :href="project.links.repo"
          target="_blank"
          rel="noopener noreferrer"
          class="p-2 text-mahogany/60 hover:text-mahogany transition-colors duration-200"
          aria-label="GitHub repository"
        >
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
            <path d="M12 0C5.374 0 0 5.373 0 12c0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.509 11.509 0 0112 5.803c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 17.3 24 12c0-6.627-5.373-12-12-12z"/>
          </svg>
        </a>
      </div>
    </div>
  </article>
</template>
